...
# Document monitor — ingest normalised docs into ChromaDB per collection:
bamboo-document-monitor \
  ...
  --once
# Use --watch DIR COLLECTION (repeatable) — one pair per ChromaDB collection.
# Each watched directory is created automatically if it does not exist.
...
**`--watch DIR COLLECTION`** (repeatable): replaces the old `--dir`/`--collection`
pair.  Each pair creates one `DocumentMonitorAgent` instance targeting the given
directory and ChromaDB collection.  Agents are ticked sequentially — no threads.
The legacy `--dir`/`--collection` flags still work (with a `DeprecationWarning`).

**Per-pair checkpoint files**: Each watch pair gets its own checkpoint file at
`.document_monitor/checkpoints_<dir_name>_<collection>.json` so that file-change
state is tracked independently per directory.

**Watched directories are created automatically**: `_start_impl` calls
`self.directory.mkdir(parents=True, exist_ok=True)`.  A missing directory is
never an error — the agent creates it and starts with an empty directory.

**`--once` flag**: Runs a single poll cycle per watch pair and exits.  Use this
in cron pipelines after `bamboo-github-sync --once`.
...
**document-monitor not picking up new files** — confirm each `--watch` directory
points at the correct `normalized_destination` subdirectory for that collection,
not the raw download directory.  Check that the directory actually exists and
contains `.md`/`.rst` files written by `bamboo-github-sync`.
